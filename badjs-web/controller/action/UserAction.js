            ...
            userDao.one({ loginName : req.body.username } ,function (err , user) {
                if(err){
                   logger.error("register user error : " + err.toString());
                    ...
                    var newUser = { chineseName: req.body.username, role : 0 ,loginName : req.body.username, email: req.body.email, password : crypto.createHash("md5").update(req.body.password).digest('hex')};
